class BlobGame extends Phaser.Scene {
  ...
  createBackgroundTexture() {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const y =
        platformCenterY - platformHeight / 2 + Math.abs(progress - 0.5) * 20;
  ...
  getRandomGroundPosition() {
    ...
    do {
      const randomX =
        platformCenterX + (Math.random() - 0.5) * platformWidth * 0.9;
      const randomY =
        platformCenterY + (Math.random() - 0.5) * platformHeight * 0.9;
  ...
  updateUI() {
    ...
    document.getElementById("blob-count").textContent = this.blobs.length;
